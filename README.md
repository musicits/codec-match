...
기기 목록 위의 '형태 전체' 드롭다운과 카드 아이콘은 `app/src/lib/form.js` 의 이름 규칙으로 갈립니다.
스마트폰은 바형 · 폴더블 · 플립, 이어폰은 무선 이어폰 · 헤드폰 둘로 나뉩니다.

이름에 '폴드'·'플립' 이 들어가지 않는 폴더블(오포 파인드 N 시리즈, 화웨이 메이트 X 시리즈 등)을
추가할 때는 `FOLD` 정규식에 이름 조각을 함께 넣어야 합니다. 안 그러면 바형으로 잡힙니다.
